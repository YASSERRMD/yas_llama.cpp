...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_DEEPSEEK_OCR:
            {
                // DeepSeek OCR shares tensor layouts with DeepSeek/DeepSeek2. The
                // text backbone may or may not use MLA depending on the upstream
                // configuration, so treat the MLA-specific metadata as optional.
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps, false);
                ml.get_key(LLM_KV_LEADING_DENSE_BLOCK_COUNT,   hparams.n_layer_dense_lead);

                const bool has_q_lora = ml.get_key(LLM_KV_ATTENTION_Q_LORA_RANK, hparams.n_lora_q, false);
                const bool has_kv_lora = ml.get_key(LLM_KV_ATTENTION_KV_LORA_RANK, hparams.n_lora_kv, false);
                const bool has_key_mla = ml.get_key(LLM_KV_ATTENTION_KEY_LENGTH_MLA,   hparams.n_embd_head_k_mla, false);
                const bool has_val_mla = ml.get_key(LLM_KV_ATTENTION_VALUE_LENGTH_MLA, hparams.n_embd_head_v_mla, false);
                ...
                const bool uses_mla =
                    (has_key_mla && has_val_mla && hparams.n_embd_head_k_mla != 0 && hparams.n_embd_head_v_mla != 0) ||
                    (!has_key_mla && !has_val_mla && (has_q_lora || has_kv_lora) && hparams.n_lora_kv != 0);

                if (!uses_mla) {
                    hparams.n_lora_q        = 0;
                    hparams.n_lora_kv       = has_kv_lora ? hparams.n_lora_kv : 0;
                    hparams.n_embd_head_k_mla = 0;
                    hparams.n_embd_head_v_mla = 0;
                    hparams.expert_gating_func = LLAMA_EXPERT_GATING_FUNC_TYPE_NONE;
                    hparams.expert_weights_norm = false;
                    hparams.rope_yarn_log_mul = 0.0f;
                } else if (hparams.expert_gating_func == LLAMA_EXPERT_GATING_FUNC_TYPE_NONE) {
                    // Align with DeepSeek2 behaviour for backwards compatibility.
                    hparams.expert_gating_func = LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX;
                }
            } break;
        case LLM_ARCH_PLM:
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        switch (arch) {
            ...
            case LLM_ARCH_DEEPSEEK_OCR:
                {
                    const bool uses_mla = (hparams.n_embd_head_k_mla != 0 && hparams.n_embd_head_v_mla != 0);
